...
<x-ui.section-card title="Gaji Pokok & Potongan" subtitle="Informasi gaji dasar dan potongan keterlambatan"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <x-ui.form-input type="number" label="Gaji Pokok" name="gaji_pokok" x-model="gajiPokok" :value="$gajiPokok ?? $user->gaji_pokok"
            prefix="Rp" variant="rounded" required />
        <x-ui.form-input type="number" label="Total Menit Terlambat" name="total_menit_telat" x-model="totalMenitTelat"
            :value="$totalMenitTelat" suffix="menit" :hint="'Diambil dari data absensi: ' . $totalMenitTelat . ' menit'" min="0" variant="rounded" required />
        <x-ui.form-input type="number" label="Potongan Per Menit" name="potongan_per_menit" x-model="potonganPerMenit"
            :value="$potonganPerMenit" prefix="Rp" :hint="'Otomatis: Rp ' .
                number_format($potonganPerMenit, 0, ',', '.') .
                '/menit (' .
                $user->gaji_pokok .
                ' ÷ ' .
                $user->jam_kerja .
                ' jam × 26 hari ÷ 60 menit, dibulatkan)'" min="0" step="1" variant="rounded"
            required />
        <x-ui.value-display label="Total Potongan Keterlambatan" variant="red"
            xText="formatNumber(totalMenitTelat * potonganPerMenit)" showSign />
