    ...
    function penggajianForm() {
        return {
            ...
            calculateParamedisInsentif() {
                const antarJemput = (parseInt(this.paramedis.antarJemputQty) || 0) * (parseFloat(this.paramedis
                    .antarJemputHarga) || 0);
                const rawatInap = (parseInt(this.paramedis.rawatInapQty) || 0) * (parseFloat(this.paramedis
                    .rawatInapHarga) || 0);
                const visit = (parseInt(this.paramedis.visitQty) || 0) * (parseFloat(this.paramedis.visitHarga) || 0);
                const grooming = (parseInt(this.paramedis.groomingQty) || 0) * (parseFloat(this.paramedis
                    .groomingHarga) || 0);
            ...
            calculateFOInsentif() {
                ...
                const appointment = (parseInt(this.fo.appointmentQty) || 0) * (parseFloat(this.fo.appointmentHarga) ||
                    0);
            ...
            calculateTechInsentif() {
                const antarKonten = (parseInt(this.tech.antarKontenQty) || 0) * (parseFloat(this.tech
                    .antarKontenHarga) || 0);
            ...
            getInsentif() {
                ...
                switch (jabatan) {
                    case 'Dokter':
                        return this.calculateDokterInsentif();
                    case 'Paramedis':
                        return this.calculateParamedisInsentif();
                    case 'FO':
                        return this.calculateFOInsentif();
                    case 'Tech':
                        return this.calculateTechInsentif();
                    default:
                        return 0;
                }
            },

            calculateGajiSubtotal() {
                const gaji = parseFloat(this.gajiPokok) || 0;
                const potonganTelat = (parseInt(this.totalMenitTelat) || 0) * (parseFloat(this.potonganPerMenit) || 0);
                const potonganLupaPulang = {{ $totalLupaPulang }} > 3 ? (parseFloat(this.potonganPerMenit) || 0) * 60 :
                    0;
                const potonganTidakHadir = (parseInt(this.totalTidakHadir) || 0) * (parseFloat(this
                    .potonganPerTidakHadir) || 0);
                const lainLain = this.calculateLainLainTotal();
                return gaji - potonganTelat - potonganLupaPulang - potonganTidakHadir + lainLain;
            },

            calculateInsentifSubtotal() {
                const insentif = this.getInsentif();
                const lembur = (parseInt(this.totalMenitLembur) || 0) * (parseFloat(this.upahLemburPerMenit) || 0);
                return insentif + lembur;
            },

            calculateTotal() {
                return this.calculateGajiSubtotal() + this.calculateInsentifSubtotal();
            }
        }
    }
</script>
