    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 animate-slide-up">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 mt-2 rounded-full text-sm font-medium
            ...
            <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-ui.form-input type="text" id="name" name="name" :value="old('name', $user->name)"
                    ...
                    <div>
                        ...
                        <x-ui.form-input type="email" id="email" name="email" :value="old('email', $user->email)"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-ui.form-input type="password" id="password" name="password"
                            placeholder="Kosongkan jika tidak ingin mengubah" variant="rounded" />
                        @error('password')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <x-ui.form-input type="password" id="password_confirmation" name="password_confirmation"
                            placeholder="Ulangi password baru" variant="rounded" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-ui.form-select name="jabatan" id="jabatan" variant="rounded" :selected="old('jabatan', $user->jabatan)"
                            :options="[
                                'Dokter' => 'Dokter',
                                'Paramedis' => 'Paramedis',
                                'Tech' => 'Tech',
                    ...
                    <div>
                        ...
                        <x-ui.form-input type="number" id="gaji_pokok" name="gaji_pokok" :value="old('gaji_pokok', $user->gaji_pokok)"
                ...
                <div>
                    ...
                    <div class="relative w-full md:w-1/2">
                        <x-ui.form-input type="number" id="jam_kerja" name="jam_kerja" :value="old('jam_kerja', $user->jam_kerja ?? 8)" suffix="jam"
                ...
                <div x-data="{
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div
                                class="w-11 h-6 bg-green-500 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-red-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-500">
                            </div>
                            <span class="ml-3 text-sm font-medium"
                                :class="isInactive ? 'text-red-700' : 'text-green-700'"
                                x-text="isInactive ? 'Inactive' : 'Active'"></span>
                        </label>
                    </div>

                    <!-- Inactive Options -->
                    <div x-show="isInactive" x-transition
                        class="space-y-4 mt-4 p-4 bg-white rounded-lg border border-gray-200">
                        <div class="flex items-center gap-4">
                        ...
                        <div x-show="isPermanent == '0'" x-transition
                            class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
                            <div>
                            ...
                            <div class="col-span-2">
                                ...
                                <x-ui.form-input type="textarea" id="inactive_reason" name="inactive_reason"
                                    :value="old('inactive_reason', $user->inactive_reason ?? '')" rows="3"
                                    placeholder="Contoh: Cuti sakit, Cuti menikah, dll." variant="rounded" />
                        ...
                        <div x-show="isPermanent == '1'" x-transition
                            class="p-4 bg-red-50 rounded-lg border border-red-200">
                            <label for="inactive_reason_permanent"
                                class="block text-sm font-medium text-gray-700 mb-2">
                                Alasan Inactive Permanen
                            </label>
                            <x-ui.form-input type="textarea" id="inactive_reason_permanent" name="inactive_reason"
                                :value="old('inactive_reason', $user->inactive_reason ?? '')" rows="3" placeholder="Contoh: Resign, Pensiun, PHK, dll."
                                variant="rounded" />
                            @error('inactive_reason')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <p class="text-sm text-gray-700">
                                <strong>⚠️ Perhatian:</strong> Pegawai yang di-inactive tidak akan dapat melakukan absen
                                dan tidak akan muncul di daftar absen harian setelah tanggal inactive. Riwayat absen
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_shift" value="1" x-model="isShift"
                                class="sr-only peer" {{ old('is_shift', $user->is_shift) ? 'checked' : '' }}>
                            <div
                            ...
                            <select id="shift_partner_id" name="shift_partner_id"
                                ...
                                @foreach ($pegawaiList as $pegawai)
                                    <option value="{{ $pegawai->id }}"
                                        {{ old('shift_partner_id', $user->shift_partner_id) == $pegawai->id ? 'selected' : '' }}>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 border-2 border-red-200 animate-slide-up-delay-2">
            ...
            <form action="{{ route('users.destroy', $user) }}" method="POST"
                ...
                @csrf
                @method('DELETE')
                <x-ui.action-button type="submit" variant="danger" iconName="trash">
                    Hapus Pegawai
                </x-ui.action-button>
            </form>
        </div>
