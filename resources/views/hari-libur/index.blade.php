        ...
        <div class="bg-white rounded-2xl shadow-xl p-6">
                                ...
                                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="p-2 {{ ($libur->tipe ?? 'libur') === 'hari_khusus' ? 'bg-blue-100' : 'bg-red-100' }} rounded-lg">
                                    ...
                                    <td class="py-3 px-4">
                                        @if (($libur->tipe ?? 'libur') === 'hari_khusus')
                                            <span
                                                class="px-2 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-700">
                                                Hari Khusus
                                            </span>
                                        @else
                                            <span
                                                class="px-2 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-700">
                                                Hari Libur
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="space-y-1">
                                            @if (($libur->tipe ?? 'libur') === 'hari_khusus' && $libur->is_masuk)
                                                ...
                                                @if ($libur->is_lembur)
                                                    <span
                                                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-orange-100 text-orange-700">
                                                        Lembur
                                                    </span>
                                                @else
                                                    <span
                                                ...
                                                @if ($libur->jam_masuk && $libur->jam_keluar)
                                                    <div class="text-xs text-gray-500">
                                                        {{ \Carbon\Carbon::parse($libur->jam_masuk)->format('H:i') }} -
                                                        {{ \Carbon\Carbon::parse($libur->jam_keluar)->format('H:i') }}
                                                    </div>
                                                @endif
                                                @if ($libur->is_shift_enabled)
                                                    <span
                                                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-purple-100 text-purple-700">
                                                        Shift
                                                    </span>
                                                @endif
                                                @if ($libur->is_lembur && $libur->upah_multiplier && $libur->upah_multiplier != 1)
                                                    <span
                                                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-yellow-100 text-yellow-700">
            ...
            @else
                <div class="text-center py-12 text-gray-500">
                    <svg class="h-16 w-16 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                        </path>
